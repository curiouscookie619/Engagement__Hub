...
def load_activity_library(path: str) -> pd.DataFrame:
    ...
    # Allow newer sample schema by mapping to the canonical columns expected by the engine
    if "activity_id" in df.columns:
        df = df.rename(
            columns={
                ...
                "eligible_life_stages": "LifeStageEligibility",
                "eligible_safari_personas": "SafariPersonaEligibility",
                "allowed_renewal_buckets": "RenewalBuckets",
                "allowed_channels": "Channels",
                "allowed_premium_to_income_bands": "EligiblePTIBands",
                "allowed_city_tiers": "EligibleCityTiers",
                "allowed_occupation_types": "EligibleOccupationTypes",
                "allowed_kids_age_bands": "EligibleKidsAgeBands",
                "business_priority": "Priority",
                "min_gap_days_same_activity": "MinSpacingDays",
            }
        )

        # Derived / default fields to satisfy required schema
        df["RequiredConsent"] = "Yes"
        df["EligibleKidsFlags"] = df.get("requires_kids", False).apply(lambda val: "Y" if str(val).upper() == "TRUE" else "ALL")
        df["MaxSurrenderPct"] = 1.0
        df["StartMonthOffset"] = 0
        df["EndMonthOffset"] = 11
        df["FrequencyMonths"] = 1

        # Normalise ALL sentinels where blank
        for column in [
            "LifeStageEligibility",
            "SafariPersonaEligibility",
            "RenewalBuckets",
            ...
            df[column] = df[column].fillna("ALL")

    _require_columns(df, ACTIVITY_REQUIRED_COLUMNS, "Activity library")
    _validate_enums(df, "RequiredConsent", ACTIVITY_CONSENT_VALUES, "Activity library")
    _validate_enums(df, "Category", ACTIVITY_CATEGORIES, "Activity library")

    numeric_fields = ["MaxSurrenderPct", "MinSpacingDays", "StartMonthOffset", "EndMonthOffset", "FrequencyMonths", "Priority"]
    _require_numeric(df, numeric_fields, "Activity library")
    if (df["MaxSurrenderPct"] < 0).any() or (df["MaxSurrenderPct"] > 1).any():
        raise ValidationError("Activity library field 'MaxSurrenderPct' must be between 0 and 1.")
